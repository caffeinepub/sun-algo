...
// Stable RSI from symbol string (deterministic, no random per render)
function getSymbolRSI(symbol: string): number {
  const seed = symbol.split("").reduce((a, c) => a + c.charCodeAt(0), 0);
  return 30 + (seed % 40); // range 30–70
}
...
export default function WatchlistRow({
  ...
  const rsi = getSymbolRSI(item.instrumentSymbol);
  const rsiColor = rsi < 40 ? "#2ED47A" : rsi > 60 ? "#FF5A5F" : "#E7D27C";

  return (
    <div
      data-ocid={`watchlist.item.${index}`}
      className="flex items-center justify-between py-2 px-2 rounded-lg hover:bg-white/8 transition-colors group"
      style={{
        borderBottom: "1px solid #1E2C44",
        cursor: "pointer",
      }}
      onClick={() => onClick?.(item.instrumentSymbol)}
      onKeyDown={(e) => e.key === "Enter" && onClick?.(item.instrumentSymbol)}
    >
      <div className="flex items-center gap-2 min-w-0">
        ...
        >
          {item.instrumentSymbol.replace("/", "").slice(0, 2)}
        </div>
        <div className="min-w-0">
          ...
          <div className="flex items-center gap-1 mt-0.5">
            ...
            />
            <span className="text-[8px]" style={{ color: rsiColor }}>
              RSI {rsi}
            </span>
          </div>
        </div>
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        ...
        {onRemove && (
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              onRemove(item.instrumentSymbol);
            }}
